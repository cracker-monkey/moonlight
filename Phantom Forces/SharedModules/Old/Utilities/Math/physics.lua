...
local function u4(p1, p2, p3, p4, p5)
	...
	if not p5 then
		if p4 then
			...
			elseif v5 < 0 then
				local v7 = (-v3) ^ 0.5;
				local v8 = mathatan2((-v5) ^ 0.5, v4) / 3;
				local v9 = v7 * mathcos(v8);
				local v10 = v7 * mathsin(v8);
...
function v1.timehit(p6, p7, p8, p9)
	local v33 = p6 - p9;
	local v34 = { u4(Vector3zero.Dot(p8, p8), 3 * Vector3zero.Dot(p8, p7), 2 * (Vector3zero.Dot(p8, v33) + Vector3zero.Dot(p7, p7)), 2 * Vector3zero.Dot(v33, p7)) };
	local v35 = 0;
	local v36 = (1 / 0);
	for v37 = 1, #v34 do
		local v38 = v34[v37];
...
function v1.trajectory(p10, p11, p12, p13)
	local v40 = p12 - p10;
	local v41 = -p11;
	local v42, v43, v44, v45 = u4(Vector3zero.Dot(v41, v41) / 4, 0, Vector3zero.Dot(v41, v40) - p13 * p13, 0, Vector3zero.Dot(v40, v40));
